...
console.log('URL da API configurada:', appConfig.apiUrl);
...
export const fetchApi = async <T>(
  ...
  try {
    ...
    console.log(`Fazendo requisição ${method} para ${url}`, {
      ...
      body: body ? JSON.stringify(body).substring(0, 100) : 'sem body',
      modo: config.mode,
    ...
    console.log(`Resposta recebida (${method} ${endpoint}):`, responseText ? responseText.substring(0, 150) + '...' : 'Vazia');
    ...
    if (error.message === 'Failed to fetch') {
      console.error('Erro de conexão - servidor possivelmente offline');
    ...
    if (endpoint.includes('/login') || endpoint.includes('/register')) {
      ...
      if (process.env.NODE_ENV === 'development' && appConfig.apiUrl.includes('localhost')) {
        console.warn('Modo de desenvolvimento detectado, tentando criar resposta mock para login');
...
export const userApi = {
  ...
  async register(userData: UserRegistrationData): Promise<ApiResponse<UserAuthResponse>> {
    console.log(`Enviando requisição de registro para ${getApiUrl()}/api/auth/register`);
    ...
    try {
      ...
      console.log(`Resposta do registro - status: ${response.status}`);
      
      // Capturar o texto da resposta para debug
      const responseText = await response.text();
      console.log(`Resposta texto (primeiros 150 chars): ${responseText.slice(0, 150)}`);
  ...
  async login(credentials: UserLoginCredentials): Promise<ApiResponse<UserAuthResponse>> {
    console.log(`Enviando requisição de login para ${getApiUrl()}/api/auth/login`);
    ...
    try {
      ...
      console.log('Login completado com sucesso:', data);
      return data as ApiResponse<UserAuthResponse>;
    } catch (error) {
  ...
  update: async (id: string, userData: Partial<User>): Promise<User> => {
    console.log('Chamando API para atualizar usuário:', id, userData);
    try {
      ...
      console.log('Resposta completa da API de atualização:', JSON.stringify(response));
      return response;
    } catch (error) {
